...
public class ProductDisplay implements Serializable {
    ...
    public StreamedContent generateCertificate(){
        ...
        if (!"created".equals(result)){
	        ...
	        BufferedReader in = new BufferedReader(new InputStreamReader(new ByteArrayInputStream(prodApplications.getRegCert())));	
	        String line = "";
			try {
				 line = in.readLine();
				 if(line.contains("PDF")){		        	
			        download  = new DefaultStreamedContent(ist, "pdf", "registration_" + prodApplications.getId() + "_" + c.get(Calendar.YEAR)+".pdf");
			     }else{
			        download  = new DefaultStreamedContent(ist, "docx", "registration_" + prodApplications.getId() + "_" + c.get(Calendar.YEAR)+".docx");
			      }          
			} catch (IOException e) {				
				e.printStackTrace();
			}	
    ...
    public String goToBack(){
    	if(userSession.getLoggedINUserID() != null && userSession.getLoggedINUserID() > 0)
    	{
    		...
    		return calcBackPage("/internal/processprodlist.faces");
    	}
        return calcBackPage("/public/productlist.faces");
    }
    /**
     * Calculate page to go back
     * @param defPage default page to go back
     * @return
     */
	public String calcBackPage(String defPage) {
