...
public class ProcessProdBnMZ implements Serializable {
	...
	public String registerProduct(ProdApplications prodApplications, boolean flag) {
		facesContext = getCurrentInstance();
		try {
			String name = "" ;
			if(prodApplications!=null && prodApplications.getProduct()!=null ){
				name = prodApplications.getProduct().getProdName().split(" ")[0] + "_registration"+ ".docx";
			}			
			if(prodApplications.getRegState().equals(RegState.REGISTERED)){		
				
				StreamedContent streamedContent = getTemplMBean().getReport(name, prodApplications.getId(),TemplateType.CERTIFICATE );
	...
	public String registerProductNA(ProdApplications prodApplications, String backTo) {
		this.registerProduct(prodApplications,true);
	...
	public String rejectProduct(ProdApplications prodApplications) {
		...
		getProcessProdBn().setProdApplications(prodApplications);
		facesContext = getCurrentInstance();
