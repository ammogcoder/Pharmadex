...
public class CustomReviewDAO implements Serializable {
	...
	public List<ReviewInfoTable> findRevFIRByUser(Long userID) {
		List<Object[]> ris = entityManager.createNativeQuery(
				...
				+ "ri.reviewStatus, ri.assignDate, rd.due_date, ri.ctdModule, ri.dueDate, ri.recomendType,"
						+"p.prod_name, pa.sra, pa.fastrack, pa.id as 'prodappId', pa.regState, ri.sec_reviewer_id, ri.secreview "
						+"FROM review_info ri "
	...
	/**
	 * Find FIR by applicant
	 * @param applicant
	 * @return
	 */
	public List<ReviewInfoTable> findFIRByApplicant(Applicant applicant) {
		List<Object[]> ris = entityManager.createNativeQuery(
				"SELECT "
				+"ri.id,"
				+"CASE " +
				"        WHEN ri.secreview='0' THEN 'PRIMARY' " +
				"        ELSE 'SECONDARY' " +
				"    END AS rev_type, " 
				+ "ri.reviewStatus, ri.assignDate, rd.due_date, ri.ctdModule, ri.dueDate, ri.recomendType,"
						+"p.prod_name, pa.sra, pa.fastrack, pa.id as 'prodappId', pa.regState, ri.sec_reviewer_id, ri.secreview "
						...
						+"left join product p on p.id=pa.PROD_ID "
						+"left join revdeficiency rd on rd.reviewinfo_ID=ri.id and rd.resolved='false' "
						+"where ri.reviewStatus='FIR_SUBMIT' and pa.APP_ID=:applId ;")
				.setParameter("applId", applicant.getApplcntId())
				.getResultList();
