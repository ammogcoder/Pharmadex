...
public class ReviewService implements Serializable {
	...
	public RetObject addReviewInfo(ReviewInfo reviewInfo) {
		...
		Date sabm = reviewInfo.getProdApplications().getSubmitDate();
		ProdApplications prodApp = prodApplicationsDAO.findProdApplications(reviewInfo.getProdApplications().getId());
		prodApp.setSubmitDate(sabm);
		prodApplicationsDAO.saveApplication(prodApp);
		return retObject;
	}
	...
	public List<ReviewInfoTable> findRevFIRByUser(Long userID) {
		...
		if(user != null){
			if(user.isCompany()){
				return customReviewDAO.findFIRByApplicant(user.getApplicant());
			}else{
