...
/*		String bankName = "";
    ...
    /**
     * Find all products "in process" for public
     * @return
     */
	public List<ProdTable> findProcessingProds() {
		return productDAO.findProcessingProducts();
	}
	/**
	 * Find submitted products for alerts
	 * If user is company user, only submitted by this company, otherwise - all
	 * @param isCompany 
	 * @param userId 
	 * @return
	 */
	public List<ProdTable> findSubmittedProds(boolean isCompany, Long userId) {
		List<ProdTable> tmp = productDAO.findProductsByState(RegState.NEW_APPL);
		List<ProdTable> ret = new ArrayList<ProdTable>();
		if(tmp != null){
			if(isCompany){
				User user = userService.findUser(userId);
				for(ProdTable item : ret){
					if(item.getAppId() == user.getApplicant().getApplcntId()){
						ret.add(item);
					}
				}
			}else{
	...
	/**
	 * Get all application for which Moderator has to assign reviewers
	 * @param moderId id of the current user, hope it is moder
	 * @return
	 */
	public List<ProdTable> findReadyForModerator(Long moderId) {
		return productDAO.findReadyForModer(moderId);
	}
	/**
	 * Find all applications that require to assign reviewers by a moderator given
	 * @param moderId
	 * @return
	 */
	public List<ProdTable> findReadyToAssignReviwers(Long moderId) {
		return productDAO.findReadyToAssignReviewers(moderId);
	}
	
	/**
	 * Find all applications in states RECOMMENDED and NOT RECOMMENDED
	 * @return
	 */
	public List<ProdTable> findReadyForHead() {
		return productDAO.findReadyForHead();
	}
	/**
	 * Find products about to expire
	 * @param isCompany for applicant only
	 * @param loggedINUserID 
	 * @return
	 */
	public List<ProdTable> findAboutToExpire(boolean isCompany, Long loggedINUserID) {
		Long applId = null;
		if(isCompany){
			applId = userService.findUser(loggedINUserID).getApplicant().getApplcntId();
		}
		return productDAO.findAboutToExpire(applId);
	}
	/**
	 * Find all just registered products
	 * @return
	 */
	public List<ProdTable> findJustRegistered() {
		return productDAO.findJustRegistered();
	}
	/**
	 * Find products that just lost registration
	 * @return
	 */
	public List<ProdTable> findJustLost() {
		return productDAO.findJustLost();
	}
